import{_ as s,c as n,o as a,W as l}from"./chunks/framework.7ce97047.js";const F=JSON.parse('{"title":"搭建一套适用于个人的CICD构建流系统","description":"","frontmatter":{"title":"搭建一套适用于个人的CICD构建流系统","date":"2023-07-21T18:00:00.000Z"},"headers":[],"relativePath":"tools/post-docker-cicd.md","filePath":"tools/post-docker-cicd.md"}'),p={name:"tools/post-docker-cicd.md"},o=l(`<p>在如今的容器服务中，使用Gitlab、K8S、Jenkins等技术部署服务是非常常见的，但这些需要的服务器内存和数量都不小。例如，部署一个Gitlab都需要至少4G内存，对于个人学习属实不太友好。</p><h2 id="介绍" tabindex="-1">介绍 <a class="header-anchor" href="#介绍" aria-label="Permalink to &quot;介绍&quot;">​</a></h2><p>基于docker，实现面向个人的CICD集成，包括：</p><ul><li>基于 docker + swarm + traefik + portainer 的容器平台。</li><li>基于 mysql + gitea + droneci + docker registry的构建平台。</li></ul><p>要求：</p><ul><li>至少1台安装好 docker 的服务器/虚拟机。备注：最好2台，一台为主服务器，一台为子服务器。</li><li>至少1个域名。备注：域名解析到国内服务器需备案</li><li>具备docker基本操作知识</li></ul><p>实践项目</p><ul><li>前后端分离项目(VueJS + NestJS)</li></ul><p>准备工作</p><ul><li>将<code>*.dev.juetan.cn</code>解析到主服务器，用于基础设施的访问</li><li>将<code>*.app.juetan.cn</code>解析到主服务器，用于部署应用的访问</li><li>在主服务器上，新建<code>/docker</code>目录</li></ul><h2 id="准备工作" tabindex="-1">准备工作 <a class="header-anchor" href="#准备工作" aria-label="Permalink to &quot;准备工作&quot;">​</a></h2><p>在开始前，需要做很多准备</p><h3 id="初始化docker集群" tabindex="-1">初始化docker集群 <a class="header-anchor" href="#初始化docker集群" aria-label="Permalink to &quot;初始化docker集群&quot;">​</a></h3><p>Docker 的 Swarm 模式表示集群， 允许在一台主服务器上，管理多台子服务器的容器和状态。</p><div class="language-bash"><button title="Copy Code" class="copy"></button><span class="lang">bash</span><pre class="shiki github-light"><code><span class="line"><span style="color:#6A737D;"># 在主服务器上，初始化docker集群</span></span>
...
<span class="line"><span style="color:#6F42C1;">docker</span><span style="color:#24292E;"> </span><span style="color:#032F62;">swarm</span><span style="color:#24292E;"> </span><span style="color:#032F62;">jorin</span><span style="color:#24292E;"> </span><span style="color:#005CC5;">--token</span><span style="color:#24292E;"> </span><span style="color:#032F62;">xx</span><span style="color:#24292E;"> </span><span style="color:#032F62;">ip:port</span></span></code></pre></div><h3 id="在主服务器上-创建必要的网络-network-。" tabindex="-1">在主服务器上，创建必要的网络(<code>network</code>)。 <a class="header-anchor" href="#在主服务器上-创建必要的网络-network-。" aria-label="Permalink to &quot;在主服务器上，创建必要的网络(\`network\`)。&quot;">​</a></h3><div class="language-bash"><button title="Copy Code" class="copy"></button><span class="lang">bash</span><pre class="shiki github-light"><code><span class="line"><span style="color:#6A737D;"># 用于加入到外部访问的公共网络</span></span>
<span class="line"><span style="color:#6F42C1;">docker</span><span style="color:#24292E;"> </span><span style="color:#032F62;">network</span><span style="color:#24292E;"> </span><span style="color:#032F62;">create</span><span style="color:#24292E;"> </span><span style="color:#005CC5;">-d</span><span style="color:#24292E;"> </span><span style="color:#032F62;">overlay</span><span style="color:#24292E;"> </span><span style="color:#032F62;">network_public</span></span></code></pre></div><h3 id="在主服务器上-创建必要的数据卷-volume-。" tabindex="-1">在主服务器上，创建必要的数据卷(<code>volume</code>)。 <a class="header-anchor" href="#在主服务器上-创建必要的数据卷-volume-。" aria-label="Permalink to &quot;在主服务器上，创建必要的数据卷(\`volume\`)。&quot;">​</a></h3><div class="language-bash"><button title="Copy Code" class="copy"></button><span class="lang">bash</span><pre class="shiki github-light"><code><span class="line"><span style="color:#6A737D;"># 用于portainer的数据卷</span></span>
...
<span class="line"><span style="color:#6F42C1;">docker</span><span style="color:#24292E;"> </span><span style="color:#032F62;">volume</span><span style="color:#24292E;"> </span><span style="color:#032F62;">create</span><span style="color:#24292E;"> </span><span style="color:#032F62;">volume_registry</span></span></code></pre></div><h3 id="在主服务器上-创建1个将分配给droneci使用的用户-用于远程更新容器" tabindex="-1">在主服务器上，创建1个将分配给<code>droneci</code>使用的用户，用于远程更新容器 <a class="header-anchor" href="#在主服务器上-创建1个将分配给droneci使用的用户-用于远程更新容器" aria-label="Permalink to &quot;在主服务器上，创建1个将分配给\`droneci\`使用的用户，用于远程更新容器&quot;">​</a></h3><div class="language-bash"><button title="Copy Code" class="copy"></button><span class="lang">bash</span><pre class="shiki github-light"><code><span class="line"><span style="color:#6A737D;"># 创建用户</span></span>
...
<span class="line"><span style="color:#22863A;">networks</span><span style="color:#24292E;">:</span></span>
<span class="line"><span style="color:#24292E;">  </span><span style="color:#22863A;">public</span><span style="color:#24292E;">:</span></span>
<span class="line"><span style="color:#24292E;">    </span><span style="color:#22863A;">external</span><span style="color:#24292E;">: </span><span style="color:#005CC5;">true</span></span></code></pre></div><ol start="2"><li>点击<code>Deploy the stack</code>按钮，等待服务部署完成，访问如下连接即可：</li></ol><div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-light"><code><span class="line"><span style="color:#24292e;">http://web.dev.juetan.cn</span></span></code></pre></div><h2 id="结语" tabindex="-1">结语 <a class="header-anchor" href="#结语" aria-label="Permalink to &quot;结语&quot;">​</a></h2><ul><li>仓库地址：<a href="https://github.com/juetan/devops-practice" target="_blank" rel="noreferrer">https://github.com/juetan/devops-practice</a></li></ul>`,58),e=[o];function c(t,r,y,i,E,d){return a(),n("div",null,e)}const A=s(p,[["render",c]]);export{F as __pageData,A as default};
