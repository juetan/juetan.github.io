...
过程宏(Procedural Macros)，是宏的高级版本，允许你扩展 Rust 的现有语法。名字中的过程，指参与编译过程，在其中修改已解析过的代码，从而达到扩展的目的。目前派生宏只能定义在声明有 `proc-macro = true` 的 crate 里，且这个 `crate` 只能导出宏，不能导出 trait 等其他数据结构。因此，写一个过程宏将需要两个 crate 的配合，如下：

- 主包：用于导出 trait 等数据结构，
- 宏包：用于导出宏。

在主包中，可以将宏这个包作为依赖再导出，后面会说这个，在开始之前我们先做一点准备。

### 前提准备

开始之前先建好项目结构，并安装好相关依赖。过程宏接收代码流，返回一个修改过的代码流，手动操作是比较麻烦的，好在社区已经有封装好的 crate，接下来安装这几个依赖：
...
2. 修改 `demo_macro/cargo.toml` 文件，将其声明为宏项目，并添加依赖。其中：syn 库用于解析代码流为 AST 结构，quote 库提供宏并将宏内容转换为代码流。
...
1. 修改 `demo_derive/src/lib.rs` 文件，添加一个派生宏，如下：

```rust
// demo_macro/src/lib.rs
